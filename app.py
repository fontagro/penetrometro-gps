...
def load_from_dropbox() -> list[dict]:
    ...
    token = st.secrets.get("DROPBOX_ACCESS_TOKEN", "").strip()
    folder = st.secrets.get("DROPBOX_FOLDER", "").strip()

    # Normalizar ruta: Dropbox API usa "" para raíz
    if folder in ("/", ".", ""):
        folder = ""
    else:
    ...
    try:
        ...
        st.sidebar.success(f"✅ Conectado como {account.name.display_name}")
        ...
        try:
            ...
            st.error(f"❌ Error listando carpeta '{folder}': {e}")
            return []
        ...
        st.sidebar.info(f"📂 {total_entries} archivos encontrados, {len(csv_files)} CSV únicos")
        ...
        st.error(f"❌ Error de entrada Dropbox: {e}. Ruta usada: '{folder}', largo token: {len(token)}")
        return []
...
def main():
    ...
    with st.sidebar:
        ...
        if source == "📂 Dropbox":
            st.markdown("---")
            # Cargar automáticamente la primera vez que alguien entra
            if 'measurements' not in st.session_state:
                st.session_state['measurements'] = load_from_dropbox()
