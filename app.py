...
def parse_penetrometro_csv(content: str, filename: str) -> list[dict]:
    ...
    Estructura:
      - Línea con Fecha = nuevo bloque de medición
      - Líneas sin Fecha = lecturas de fuerza a profundidad
      - Líneas con Distancia total / Fuerza Promedio / Fuerza Maxima = resumen
    
    Retorna lista de mediciones con GPS, fuerzas, y perfil completo.
    """
    ...
    for line in lines[1:]:  # Skip header
        ...
        if fecha:
            ...
            current = {
                ...
                'medicion': medicion,
                'archivo': filename,
                'readings': [],          # lista de (fuerza, distancia)
                'fuerza_max': None,
                'fuerza_min': None,
                'fuerza_prom': None,
                'dist_total': None,
        ...
        # ── Línea de resumen ──
        if dist_total and fuerza_max:
            try:
                fm = float(fuerza_max.replace(',', '.'))
                fp = float(fuerza_prom.replace(',', '.')) if fuerza_prom else 0
                dt = int(dist_total)
                dfm = int(dist_fmax) if dist_fmax else 0

                # Ignorar líneas de resumen vacías (fuerza_max = 0)
                if fm <= 0:
                    continue
                ...
                if current['fuerza_prom'] is None or fp > 0:
...
def _finalize_measurement(m: dict):
    ...
    # Calcular min/max de lecturas individuales
    if m['readings']:
        ...
        # Si no hay línea de resumen, usar lecturas
        if m['fuerza_max'] is None:
            m['fuerza_max'] = max(fuerzas)
            m['fuerza_prom'] = round(sum(fuerzas) / len(fuerzas), 2)
            m['fuerza_min'] = min(fuerzas)
    else:
        m['fuerza_min_lectura'] = None
        m['fuerza_max_lectura'] = None
        m['cant_lecturas'] = 0

    if m['fuerza_min'] is None and m['readings']:
        m['fuerza_min'] = min(r[0] for r in m['readings'])
    ...
    m['valid'] = m['fuerza_max'] is not None and m['fuerza_max'] > 0
...
def load_from_dropbox() -> list[dict]:
    """Carga CSVs desde carpeta de Dropbox."""
    token = st.secrets.get("DROPBOX_ACCESS_TOKEN", "")
    folder = st.secrets.get("DROPBOX_FOLDER", "/Penetrometro")
    ...
    try:
        ...
        progress = st.progress(0, text="Leyendo archivos de Dropbox...")
...
def main():
    ...
    with st.sidebar:
        ...
        if source == "📂 Dropbox":
            st.markdown("---")
            st.markdown(
                "Configurá el token en `.streamlit/secrets.toml`:\n"
